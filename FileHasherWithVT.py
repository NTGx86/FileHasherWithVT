...
This program can run in two modes; the first will accept command-line args to accomodate hash generation for specific
files or for entire directories of files. The second being an user prompt mode if no additional
command line arguments are provided. File hashes for both modes are displayed by using a pretty table. 


Virus Total (VT) Notes: 

*For obvious reasons, like preventing abuse related to my own personal VT API key, I have ommitted mine
from this script. You should place your own VT API key in it's place. 

“The requested resource is not among the finished, queued or pending scans” >> means that the file's hash or file 
...
cybersecurity engineering coursework at the University of Arizona. With that in mind, many of the functionality 
is not novel or groundbreaking, but simply a method for me to apply my skills and have fun with python.
...
targetDir = os.path.join(parentDirPath, userProvidedDir)
...
for root, dirs, files in os.walk(targetDir): 
    ...
    ''' File Hashing: process each file in the TARGET Directory '''
        ...
        if eachFile != ".DS_Store":
            try:
                ...
                with open(fullPath, 'rb') as fileToHash:
                    ...
                    ''' create a pretty table for each file's file hashes '''
                    hashTable = PrettyTable(["Hash Type", "File Hash"])                    
                    ...
                    hashTable.add_row(["MD5",fileHashMD5])
                    
                    fileHashSHA1 = hashlib.sha1(buffer).hexdigest() 
                    hashTable.add_row(["SHA-1",fileHashSHA1])
                    
                    fileHashSHA256 = hashlib.sha256(buffer).hexdigest() 
                    hashTable.add_row(["SHA-256",fileHashSHA256]) 
                    
                    fileHashSHA512 = hashlib.sha512(buffer).hexdigest() 
                    hashTable.add_row(["SHA-512",fileHashSHA512])   
                    #print("="*160)
